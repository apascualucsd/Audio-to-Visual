...
void drawSpectrogram(uint16_t color) {
  ...
  for (int x = 1; x < SAMPLES; x = x + 8) {
    ...
    int distance_from_middle = abs(x - 256);
    int max_intensity = map(distance_from_middle, 0, 256, 32, 16);

        // Map frequency to color and draw on matrix
      int intensity = map(vReal[x], 0, 1000, 0, max_intensity);
      // Constrain the intensity to ensure it stays within the calculated range
